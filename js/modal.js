...
function abrirModalProduto(produto) {
  ...
  // Sincronizar com os botões de sabor para sorvetes (seleção única)
  if (produto.nome === 'Sorvetes') {
    const saborButtons = document.querySelectorAll('#modalOptions .option-btn[data-name="Sabor"]');
    saborButtons.forEach(button => {
      button.addEventListener('click', () => {
        // Desmarcar todos os outros botões de sabor
        ...
        if (index !== -1) {
          modalImagem.src = sorveteImages[index];
          modalNome.textContent = sorveteNames[index];
        }
      });
    });
  }
